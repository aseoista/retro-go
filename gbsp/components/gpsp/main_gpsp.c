...
// ─── Profiling counters (enabled with -DGBSP_PROFILE) ──────────────────────
#ifdef GBSP_PROFILE
static int64_t prof_timer_us = 0;
static int64_t prof_scanline_us = 0;
static int64_t prof_sound_us = 0;
static uint32_t prof_update_calls = 0;
static uint32_t prof_scanline_calls = 0;
...
#ifdef GBSP_PROFILE
    prof_timer_us += esp_timer_get_time() - _t1; }
#endif
...
/*          printf("%d gbc audio updates\n", gbc_update_count);
...
#ifdef GBSP_PROFILE
          { int64_t _t3 = esp_timer_get_time();
#endif
          render_gbc_sound();
#ifdef GBSP_PROFILE
          prof_sound_us += esp_timer_get_time() - _t3; }
#endif
          ...
          prof_frames++;
          if (prof_frames >= 60) {
            int64_t tot = prof_timer_us + prof_scanline_us + prof_sound_us;
            // prof_other_us = update_gba overhead not in the above
            printf("[PROF] upd_calls=%u scan_calls=%u timer=%lldus scan=%lldus sound=%lldus tot_in_upd=%lldus\n",
                   (unsigned)prof_update_calls, (unsigned)prof_scanline_calls,
                   prof_timer_us/60, prof_scanline_us/60, prof_sound_us/60, tot/60);
            prof_timer_us = prof_scanline_us = prof_sound_us = 0;
            prof_update_calls = prof_scanline_calls = prof_frames = 0;
          }
#endif
        }
